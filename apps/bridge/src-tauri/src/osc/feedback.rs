...
//! desktops / OSCPoint send oscpoint feedback messages (`/oscpoint/slideshow/*`,
//! `/oscpoint/v2/event`, etc.) to it. We cache the latest state in
//! [`FeedbackState`] and broadcast updates via [`FeedbackUpdate`] to anyone
//! who wants streaming updates (the Tauri frontend, the HTTP config API).
...
///
/// All fields are optional — the bridge does not know what's been heard from
/// any desktop until one sends a feedback message.
...
pub struct FeedbackState {
    ...
    /// Last outgoing OSC command sent (mirrors Python bridge's `last_command`).
...
impl FeedbackState {
    /// Apply an inbound OSC message to the cached state. Returns `true` if the
    /// state changed (so the caller knows whether to broadcast an update).
    fn apply_message(&mut self, msg: &OscMessage) -> bool {
        let changed = match msg.addr.as_str() {
            ...
            addr if addr.starts_with(messages::feedback::V2_STATE_PREFIX) => {
                // oscpoint v2 state broadcast — sub-address after the prefix.
                // Best-effort parsing of the trailing segment, e.g.
                //   /oscpoint/state/presenting  -> presenting: bool
                //   /oscpoint/state/open         -> open: bool
    ...
    pub fn remember_outgoing_command(&mut self, address: &str) {
        // Store the short name (after the last slash) for UI display.
...
/// background task on shutdown via [`FeedbackListenerHandle::cancel`].
pub struct FeedbackListenerHandle {
    pub cancel: broadcast::Sender<()>,
}

/// Spawn a tokio task that listens for OSC feedback packets on `0.0.0.0:port`
/// and updates the shared cache. Returns immediately with a handle.
///
/// Packets are parsed with `rosc` and dispatched to [`FeedbackState::apply_message`].
/// On change, the new snapshot is broadcast on `update_tx`.
pub fn start_feedback_listener(
    ...
    // Bind a synchronous UDP socket, since rosc::dispatcher works on bytes.
    // The recv loop is fully blocking (no `.await`), so we run it on a dedicated
    // OS thread rather than a tokio task — this also means it works when called
    // synchronously from Tauri's `setup()`, which is not inside a runtime context.
    ...
    std::thread::spawn(move || {
        ...
        loop {
            // Co-operative cancel via timeout polling: every 200ms the recv
            // times out and we check the cancel channel.
...
// =============================================================================
// Helper trait extensions for rosc::OscType
// =============================================================================
