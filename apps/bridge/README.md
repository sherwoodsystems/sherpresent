...
The bridge comes pre-configured for **broadcast mode** on channel `main`. No IP configuration needed - it automatically broadcasts to all sher-present instances on your network.

Just ensure sher-present on your presenter PC is configured:
...
For advanced settings, access the web interface at:
```
http://<raspberry-pi-ip>/
http://clicker-2.local/
```
...
Valid channels: `main`, `backup`, `keynote1`-`keynote9`, `aux1`-`aux9`
...
|--------|-------------|---------|
| `version` | Config format version (always `2`) | `2` |
| `broadcast_mode` | Use UDP broadcast instead of direct IP | `true` |
| `channel` | Channel name for broadcast mode | `main` |
| `broadcast_port` | Port for broadcast communication | `9002` |
| `feedback_port` | Port for receiving presentation feedback | `9002` |
| `log_level` | Logging verbosity: `DEBUG`, `INFO`, `WARNING`, `ERROR` | `INFO` |
| `devices` | Device registrations for multi-device mode (advanced) | `{}` |
...
The web interface runs automatically on port 80 (standard HTTP) and provides an easy way to configure the bridge without SSH access.
...
|-----|-------------|-------------|
| Left Arrow | `/clicker/{channel}/prev` | Previous slide on channel |
| Right Arrow | `/clicker/{channel}/next` | Next slide on channel |

Example with channel `main`: `/clicker/main/next`
